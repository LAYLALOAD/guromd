...
public class C0531_07 {
	...
	public static void main(String[] args) {
		...
		//System.out.printf("%d,%s,%d,%d,%d",list.get(0).getStuNo(),list.get(0).getName(),list.get(0).getKor(),list.get(0).getEng(),list.get(0).getMath());
		
		
		// 파일저장
		String data = "";
		for(int i=0;i<list.size();i++) {
			data += String.format("%d,%s,%d,%d,%d\r\n",list.get(i).getStuNo(),list.get(i).getName().trim(),list.get(i).getKor(),list.get(0).getEng(),list.get(0).getMath());
		}
		...
		FileWriter fw = null;
		try {
			fw = new FileWriter("c:/javaclass/savedata/student.txt");
			fw.write(data);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
