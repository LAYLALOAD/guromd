...
public class StuMain {
	...
	public static void main(String[] args) {
		...
		//Student[] s = new Student[10]; //배열선언 (객체선언X)
			...
			case 3: //성적수정
				...
				for(int i=0; i<list.size();i++) {
					...
					if(s.getName().equals(searchName)) {
						...
						System.out.printf("[ %s점수가 수정 되었습니다. ]\n",Student.title[choice+1]);
						System.out.println();
					    chk = 1;
	...
	static void stuOutput(ArrayList<Student> list) {
		System.out.printf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t \n",Student.title[0],Student.title[1],Student.title[2],Student.title[3],Student.title[4],Student.title[5],Student.title[6],Student.title[7]);
		System.out.println("-------------------------------------------------------------");
		for(int i=0;i<list.size();i++) {
			//"학번", "이름", "국어", "영어", "수학", "합계", "평균", "등수"
			Student s = list.get(i);
			System.out.printf("%s\t%s\t%d\t%d\t%d\t%d\t%.2f\t%d\t \n",list.get(i).getStuNo(),s.getName(),s.getKor(),s.getEng(),s.getMath(),s.getTotal(),s.getAvg(),s.getRank());
		}
		System.out.println();
	}//stuOutput
