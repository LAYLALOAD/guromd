...
public class FController extends HttpServlet {
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String uri = request.getRequestURI(); //	/jsp0621_jspMall/login.do
		String contextPath = request.getContextPath(); //	/jsp0621_jspMall
		...
		if(fileName.equals("/list.do")) {
			...
			bservice = new BListService(); //다형성, list가져옴
			bservice.execute(request, response);
			page = "list.jsp";
		}else if(fileName.equals("/view.do")){
			bservice = new BViewService(); //board 가져옴
			bservice.execute(request, response);
			page = "view.jsp";
